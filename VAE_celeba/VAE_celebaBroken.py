...
train_loader = range(1, 500)
test_loader = range(1, 128)
...
class VAE(nn.Module):
    ...
    def reparameterize(self, mu, logvar):
        if self.training:
            ...
            return eps.mul(std).add_(mu) # return the latent vector (sample from standard deviations and add the mean)
        else:
            return mu 
    #
    ...
    def forward(self, x):
        ...
        z = self.reparameterize(mu, logvar) # z is the latent vector
...
def train(epoch):
    ...
    for batch_idx in train_loader:
        ...
        loss.backward() # calculate gradients: d(loss)/d(x)
        train_loss += loss.item() # .item extracts number value from the tensor
        optimizer.step()
        if batch_idx % args.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                100. * batch_idx / len(train_loader),
                loss.item() / len(data)))  # len(data) is batch size
