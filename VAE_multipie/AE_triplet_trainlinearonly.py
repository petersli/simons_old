...
class Disentangler(nn.Module):
	def __init__(self, latent_variable_size):
		...
		self.Linear2 = nn.Linear(latent_variable_size, latent_variable_size / 2)
	#	self.Linear3 = nn.Linear(latent_variable_size, latent_variable_size)

		self.sigmoid = nn.Sigmoid()

	def forward(self, z):
		z_per = self.sigmoid(self.Linear1(z))
		z_exp = self.sigmoid(self.Linear2(z))
	#	z_concat = self.sigmoid(self.Linear3(torch.cat((z_per, z_exp), dim=1)))
		z_concat = torch.cat((z_per, z_exp), dim=1)
